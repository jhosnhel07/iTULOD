...
-- ---------------------------------------------------------------------
-- default admin account: username "admin" / surname "admin" / password "admin"
...
begin
  ...
  values (
    ...
    '{"full_name":"admin admin","role":"admin"}',
    now(),
    now()
  )
  ...
  insert into public.profiles (id, full_name, email, role)
  values (admin_id, 'admin admin', 'admin@itulod.local', 'admin')
  on conflict (id) do update
    set role = 'admin', full_name = 'admin admin', email = 'admin@itulod.local';
end $$;
...
-- profiles ------------------------------------------------------------
-- A user can read their own profile, an admin can read all, OR a customer
-- can read the profile of any rider currently assigned to one of their own
-- transport/food/parcel bookings (needed for the booking details modal).
...
-- rider_applications ---------------------------------------------------
-- Same idea: a customer can see the vehicle/plate/license details of the
-- rider assigned to their own booking, not just the rider themself or admin.
...
create policy "payments_select" on public.payments
  for select using (customer_id = auth.uid() or rider_id = auth.uid() or public.is_admin());
-- The rider (not the customer) is the one who inserts this row when they
-- mark a booking complete, so the check must also allow rider_id = auth.uid().
create policy "payments_insert" on public.payments
  for insert with check (customer_id = auth.uid() or rider_id = auth.uid() or public.is_admin());
